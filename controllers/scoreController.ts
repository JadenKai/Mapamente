...
async function supply10FromCityAndPersonalBest(req: Request, res: Response) {
  ...
  const top10 = await Promise.all(
    (await Model.pullTop10OfCity(cityId)).map((entry) =>
      convertScoreEntryToScore(entry),
    ),
  );
  ...
  res.render("leaderboardTemplate", {
    scores: top10,
...
async function getTop10OfUser(userId: number): Promise<Score[]> {
  ...
  const temp = Promise.all(
    (await Model.pullTop10OfUser(userId)).map((entry) =>
      convertScoreEntryToScore(entry),
    ),
  );
  return temp;
...
async function homePage(req: Request, res: Response) {
  res.render("home", {
    scores: await Promise.all(
      (await Model.pullTop10Total()).map((entry) =>
        convertScoreEntryToScore(entry),
      ),
    ),
  });
...
export { supply10FromCityAndPersonalBest, getTop10OfUser, homePage };

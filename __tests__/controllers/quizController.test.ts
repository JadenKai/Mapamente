...
const mockCheckCorrectAnswer = quizModel.checkCorrectAnswer as jest.MockedFunction<typeof quizModel.checkCorrectAnswer>;
const mockPollQuizQuestions = quizModel.pollQuizQuestions as jest.MockedFunction<typeof quizModel.pollQuizQuestions>;
const mockPollAnswersForQuestion = quizModel.pollAnswersForQuestion as jest.MockedFunction<typeof quizModel.pollAnswersForQuestion>;
const mockGetCityById = getCityById as jest.MockedFunction<typeof getCityById>;
const mockGetCityIdByQuestionId = getCityIdByQuestionId as jest.MockedFunction<typeof getCityIdByQuestionId>;
...
describe('returnQuiz', () => {
  ...
  it('scores correctly when time is above the 10000ms minimum', async () => {
    ...
    expect(mockUploadScore).toHaveBeenCalledWith(
      expect.objectContaining({ correctCount: 2, score: expectedScore, userId: 5, cityId: 3 })
    );
    expect(res.render).toHaveBeenCalledWith('landing', expect.objectContaining({ scoreEntry: expect.any(Object) }));
  });
...
describe('produceQuiz', () => {
  ...
  it('renders quizTemplate with questions and city name', async () => {
    ...
    mockGetCityById.mockResolvedValue({ cityName: 'TestCity', cityId: 1 } as any);
  ...
  it('fetches answers for each question independently', async () => {
    ...
    mockPollAnswersForQuestion.mockResolvedValue([{ answerId: 99, answerText: 'A' } as any]);

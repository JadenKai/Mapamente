...
const mockPullTop10OfCity = scoreModel.pullTop10OfCity as jest.MockedFunction<
  typeof scoreModel.pullTop10OfCity
>;
const mockPullTop10OfUser = scoreModel.pullTop10OfUser as jest.MockedFunction<
  typeof scoreModel.pullTop10OfUser
>;
const mockPullTop10Total = scoreModel.pullTop10Total as jest.MockedFunction<
  typeof scoreModel.pullTop10Total
>;
const mockGetPersonalBest = scoreModel.getPersonalBest as jest.MockedFunction<
  typeof scoreModel.getPersonalBest
>;
...
describe("supply10FromCityAndPersonalBest", () => {
  it("renders leaderboard with top 10 scores and city name", async () => {
    ...
    expect(res.render).toHaveBeenCalledWith("leaderboardTemplate", {
      scores: [
        { username: "alice", correctCount: 8, score: 5000, cityName: "Rome" },
      ],
      cityName: "Rome",
...
describe("getTop10OfUser", () => {
  it("returns an array of Score objects for a given user", async () => {
    mockPullTop10OfUser.mockResolvedValue([
      fakeEntry(),
      fakeEntry({ score: 3000, correctCount: 5 }),
    ]);

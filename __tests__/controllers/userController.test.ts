import { signupPost, loginPost, profileGet } from '../../controllers/userController.js';
...
const mockGetTop10OfUser = getTop10OfUser as jest.MockedFunction<typeof getTop10OfUser>;
...
const fakeUser = (overrides = {}) => ({
  userId: 1, username: 'alice', passwordHash: 'hashed', profilePic: null,
  isPublic: true, isAdmin: false, themeLight: true, isActive: true,
  ...overrides,
} as any);
...
describe('signupPost', () => {
  it('renders signup error when any required field is missing', async () => {
    const req = makeReq({ body: { username: 'alice', password: '', passwordCheck: '' } });
    const res = makeRes();

    await signupPost(req, res);

    expect(res.render).toHaveBeenCalledWith('signup', { error: 'All fields are required.' });
    expect(mockCreateUser).not.toHaveBeenCalled();
  });

  it('renders signup error when passwords do not match', async () => {
    const req = makeReq({ body: { username: 'alice', password: 'abc', passwordCheck: 'xyz' } });
    const res = makeRes();

    await signupPost(req, res);

    expect(res.render).toHaveBeenCalledWith('signup', { error: 'Passwords do not match.' });
  });

  it('renders signup error when username is already taken', async () => {
    const req = makeReq({ body: { username: 'alice', password: 'pass', passwordCheck: 'pass' } });
    const res = makeRes();
    ...
    expect(res.render).toHaveBeenCalledWith('signup', { error: 'Username is already taken.' });
    expect(mockCreateUser).not.toHaveBeenCalled();
  });

  it('creates user, sets session, and redirects on success', async () => {
    const req = makeReq({ body: { username: 'bob', password: 'pass', passwordCheck: 'pass' } });
    const res = makeRes();

    // signupPost calls findUserByUsername twice: once to check availability, once
    // to read back the newly created user's id before setting the session.
...
describe('loginPost', () => {
  it('renders login error when any field is missing', async () => {
    ...
    expect(res.render).toHaveBeenCalledWith('login', { error: 'All fields are required.' });
  });
...
describe('profileGet', () => {
  ...
  it('renders profile with user data and scores', async () => {
    ...
    const res = makeRes();
    const user = fakeUser();
    const scores = [{ username: 'alice', correctCount: 9, score: 8000, cityName: 'Paris' }];

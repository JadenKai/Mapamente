...
async function addQuestion(question: QuestionEntry): Promise<boolean> {
  const [result] = await pool.query<ResultSetHeader>(
    ...
    [question.questionId, question.cityId, question.questionText],
  );
...
async function pollQuizQuestions(cityId: number): Promise<QuestionEntry[]> {
  const rows = (
    await pool.query<QuestionEntry[]>(
      "SELECT * FROM Question WHERE cityId = ? ORDER BY RAND() LIMIT 10;",
      [cityId],
    )
  )[0];
  return rows;
...
async function pollAnswersForQuestion(
  questionId: number,
): Promise<AnswerEntry[]> {
  const rows = (
    await pool.query<AnswerEntry[]>(
      "SELECT * FROM Answer WHERE questionId = ? ORDER BY RAND();",
      questionId,
    )
  )[0];
  return rows;

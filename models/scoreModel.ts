...
async function pullTop10OfUser(userId: number): Promise<ScoreEntry[]> {
  const rows = (
    await pool.query<ScoreEntry[]>(
      "SELECT * " + "FROM Score WHERE userId = ? ORDER BY score DESC LIMIT 10",
      [userId],
    )
  )[0];
  return rows;
...
async function pullTop10OfCity(cityId: number): Promise<ScoreEntry[]> {
  const rows = (
    await pool.query<ScoreEntry[]>(
      "SELECT * FROM Score WHERE cityId = ? AND isPublic = true ORDER BY score DESC LIMIT 10",
      [cityId],
    )
  )[0];
  return rows;
...
async function getPersonalBest(
  ...
  const [rows] = await pool.query<ScoreEntry[]>(
    "SELECT * FROM Score WHERE userId = ? AND cityId = ?",
    [userId, cityId],
  );
  return rows[0];
}
